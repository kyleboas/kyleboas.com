...
The separate Cloudflare Worker `infiniteflight-inbounds` serves shared public Inbounds snapshots at `kyleboas.com/api/infiniteflight/*`. Its scoped route lets the static page at `kyleboas.com/infiniteflight/inbounds/` call the API without exposing the server-side `INFINITEFLIGHT_API_KEY`. It is a constrained proxy, not a general upstream pass-through: it exposes only the configured session, flights, ATC, world, and validated airport metadata/status/ATIS resources required by the restored UI. A singleton Durable Object coalesces and caches each resource globally, so visitors share snapshots rather than generating per-user upstream requests.
